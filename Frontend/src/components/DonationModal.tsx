...
interface DonationModalProps {
  ...
  onEditClick: (donation: Donation) => void;
  onDeleteClick: (donationId: string) => void;
  isAdmin: boolean; // Include the isAdmin prop here
}
...
const DonationModal: React.FC<DonationModalProps> = ({
  ...
  onEditClick,
  onDeleteClick,
  isAdmin, // Destructure isAdmin prop
}) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editData, setEditData] = useState<Donation | null>(null);
  const [errors, setErrors] = useState({
    ...
    >
      ...
      <Modal.Footer>
        ...
        {isEditing ? (
          ...
          (editData.approvedByAdmin === 'false') && !["הגיע לעמותה", "נאסף", "נמסר בעמותה"].includes(editData.status) && (
            <Button variant="primary" onClick={handleEdit}>
              ערוך
            </Button>
          )
