...
function MainPage() {
    ...
    req
      .then((res) => {
        ...
        const categoryCounts = {
          food: res.data.filter((product) => product.category === "מזון ושתייה")
            .length,
          clothing: res.data.filter((product) => product.category === "ביגוד")
            .length,
          furniture: res.data.filter((product) => product.category === "ריהוט")
            .length,
          footwear: res.data.filter((product) => product.category === "הנעלה")
            .length,
          babyGear: res.data.filter(
            (product) => product.category === "ציוד לתינוקות"
          ).length,
          houseware: res.data.filter(
            (product) => product.category === "כלי בית"
          ).length,
          books: res.data.filter((product) => product.category === "ספרים")
            .length,
          toys: res.data.filter((product) => product.category === "צעצועים")
            .length,
          other: res.data.filter((product) => product.category === "אחר")
            .length,
        };
  ...
  const numUsers = users.filter(
    (user) => user.rating === "⭐⭐⭐⭐⭐" && user.isPublished
  ).length;
  ...
  const totalDonations = Object.values(counts).reduce(
    (acc, count) => acc + count,
    0
  );
    ...
    >
      ...
      <Box
        ...
        sx={{
          ...
          background:
            "linear-gradient(180deg, rgba(249, 230, 167, 0.8) 10%, rgba(245, 245, 244, 0.5) 100%)",
      ...
      <Box
        component="section"
        className="section-centered animated-section"
      ...
      >
        <Container>
          ...
          >
            עמותת ואהבתם ביחד (ע.ר 580776359) מסייעת לאוכלוסיות נזקקות ומפעילה
            בין היתר את המכולת החברתית, המקרר השיתופי, תיק לכל ילד וקיר המעילים
          </Typography>
          <Typography
            variant="body1"
            sx={{ textAlign: "center", marginTop: "20px" }}
      ...
      <Box className="section-section-light animated-section">
        ...
        {requests.length === 1 ? (
          <Box sx={{ p: 1, textAlign: "center" }}>
            ...
            <Typography
              variant="body2"
              sx={{ mt: 2, fontWeight: "bold", fontFamily: "Assistant" }}
            >
              {requests[0].itemName}
            </Typography>
      ...
      <Box
        className="section-section-light animated-section"
      ...
      <Box
        className="section-section3-light animated-section"
        sx={{ mb: 5, fontFamily: "Assistant" }}
      >
        <Typography
          variant="h3"
          sx={{
        ...
        {numUsers === 1 ? (
          users
            .filter((user) => user.rating === "⭐⭐⭐⭐⭐" && user.isPublished)
            .map((user, index) => (
              <Box
                key={index}
                sx={{ p: 1, textAlign: "center", alignItems: "center" }}
              >
                <Avatar
      ...
      <Box
        className="section-partners animated-section"
        sx={{
      ...
      >
        ...
        >
          <Box
            ...
            src={logo10}
            alt="logo10"
            sx={{ maxWidth: { xs: "150px", sm: "200px", md: "250px" } }}
          />
          <Box
            component="img"
            src={logo11}
      ...
      {/* Section 4: Donations and Community Counters */}
      <Box
        className="section-counters animated-section"
        sx={{ marginTop: "50px", padding: "50px 0" }}
      >
        <Typography
          variant="h3"
          sx={{
        ...
        >
          <Grid item xs={12} sm={6} md={4} sx={{ textAlign: "center" }}>
            <VisibilitySensor
              partialVisibility
              offset={{ bottom: 200 }}
              onChange={handleDonationsVisibility}
            >
              <div>
                <Typography variant="h3" sx={{ mb: 1 }}>
                  <CountUp
                    end={hasAnimatedDonations ? totalDonations : 0}
                    duration={2}
                  />
                </Typography>
                <img
                ...
                <Typography
                  variant="h6"
                  sx={{ mt: 1, whiteSpace: "nowrap", textAlign: "center" }}
                >
                  תרומות שנתרמו
                </Typography>
              </div>
          ...
          <Grid item xs={12} sm={6} md={4} sx={{ textAlign: "center" }}>
            <VisibilitySensor
              partialVisibility
              offset={{ bottom: 200 }}
              onChange={handleUsersVisibility}
            >
              <div>
                <Typography variant="h3" sx={{ mb: 1 }}>
                  <CountUp
                    end={hasAnimatedUsers ? users.length : 0}
                    duration={2}
                  />
                </Typography>
                <img
