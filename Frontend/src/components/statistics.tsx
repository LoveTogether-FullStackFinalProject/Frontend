...
const Statistics = () => {
  ...
  const chartData =
    selectedChart === "users"
      ...
      : getTopN(
          aggregateData(
            selectedChart === "requests" ? requests : products,
            xAxisField
          ),
          10
        );
      ...
      <Container className="statistics-page" dir="rtl">
                          ...
                          <TableRow key={index}>
                            <TableCell style={{ textAlign: "right" }}>
                              {topProducts[index]?.name &&
                                `${topProducts[index].name} (${topProducts[index].count} תרומות)`}
                            </TableCell>
                            <TableCell style={{ textAlign: "right" }}>
                              {topUsers[index]?.name &&
                                `${topUsers[index].name} (${topUsers[index].count} תרומות)`}
                            </TableCell>
                            <TableCell style={{ textAlign: "right" }}>
                              {topCategories[index]?.name &&
                                `${topCategories[index].name} (${topCategories[index].count} תרומות)`}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
        ...
        <Box mb={4}>
          <FormControl sx={{ m: 1, minWidth: 120, padding: 2 }}>
            <InputLabel sx={{ textAlign: "right", width: "100%", padding: 1 }}>
              שדה X
            </InputLabel>
            <Select
              value={xAxisField}
              onChange={handleXAxisFieldChange}
              disabled={selectedChart === "users"}
            >
